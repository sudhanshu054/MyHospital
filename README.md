...
### Render (recommended — frontend + backend fully hosted)

The frontend (React static site) and backend (Spring Boot Docker service) both run on
Render. The database is **Supabase PostgreSQL** and the cache is **Upstash Redis** —
both have free tiers with no 30-day expiry.

Follow [docs/render-deployment.md](docs/render-deployment.md) for the full step-by-step
guide. In short:

1. Create a **Supabase** project and note the session-pooler host, user, and password.
2. Create an **Upstash Redis** database and note the endpoint and password.
3. **Render → New → Blueprint** → connect this repo → Render reads `render.yaml` and
   creates `hospital-frontend` and `hospital-backend`.
4. In the Render dashboard, fill the `sync: false` secrets for the backend (database
   credentials, Redis credentials, `JWT_SECRET`, `GOOGLE_OAUTH_CLIENT_ID`,
   `OPENAI_API_KEY`) and for the frontend (`VITE_GOOGLE_OAUTH_CLIENT_ID`).
5. Every push to `master` auto-deploys both services.

Key backend environment variables set automatically by `render.yaml`:
...
| `CORS_ALLOWED_ORIGINS` | `https://hospital-frontend.onrender.com` |

### Docker Compose (self-hosted / local)
