...
const origin = (process.env.API_ORIGIN || process.env.TUNNEL_API_ORIGIN || process.env.KOYEB_API_ORIGIN || process.env.RENDER_API_ORIGIN || process.env.CLOUD_RUN_API_ORIGIN || process.env.CLOUDFLARE_API_ORIGIN || '').replace(/\/$/, '');

if (!origin) {
  console.error('Set API_ORIGIN or TUNNEL_API_ORIGIN before deploying Pages.');
  process.exit(1);
}
